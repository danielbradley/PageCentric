...
IF Is_Local_Caller() THEN
    ...
    REPLACE INTO articles
        (  ARTICLE,  modified,  id,  source,  category,  subject,  date,  title,  video_type,  video_code,  h1,  address,  section,  section_references )
    VALUES
        ( $ARTICLE, $modified, $id, $source, $category, $subject, $date, $title, $video_type, $video_code, $h1, $address, $section, $section_references );
...
IF Is_Local_Caller() THEN
    ...
    SET $contains = EXISTS( SELECT * FROM articles_info WHERE modified < $modified );
