	...
	function __construct( $request, $required )
	{
		...
		if ( is_array( $this->request ) && array_key_exists( "action", $this->request ) )
		{
			...
			foreach ( $this->required as $name => $required )
			{
				...
				{
				case "ALPHA":
					$value = array_get( $name, $this->request );
				...
				case "ALPHAwSPACES":
					$value = str_replace( ' ', '', array_get( $name, $this->request ) );
					if ( ("" == $value) || !ctype_alpha( $value ) )
					{
						$this->required["$name"] = $text;
					}
				...
				case "POSTCODE_AU":
					$value = array_get( $name, $this->request );
					if ( ("" == $value) || (4 > strlen( $value)) || (4 < strlen( $value )) || !ctype_digit( $value ) )
					{
						$this->required["$name"] = $text;
					}
					else
		...
		function isAXN( $value )
		{
			...
			return ( is_numeric( $value ) && ((9 == strlen( $value )) || (11 == strlen( $value ))) );
		}
