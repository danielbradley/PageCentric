...
END
;
CREATE PROCEDURE Payments_Replace
(
  $sid                             CHAR(32),
  $USER                             INT(11),
  $final_four                      CHAR(4),
...
IF $USER = @USER THEN
  ...
  SELECT created     INTO @created     FROM payments WHERE USER=$USER;
  SELECT customer_id INTO @customer_id FROM payments WHERE USER=$USER;

  REPLACE INTO payments
    (  USER,  created,  customer_id,  final_four,  number,  cvv,  month,  year )
  VALUES
    ( $USER, @created, @customer_id, $final_four, $number, $cvv, $month, $year );
...
END
;
CREATE PROCEDURE Payments_Delete
(
  $sid                             CHAR(32),
  $USER                             INT(11)
)
...
IF $USER = @USER THEN
  ...
  UPDATE payments SET final_four='', number='', cvv='', month='', year=''
  WHERE USER=$USER;
...
END
;
CREATE PROCEDURE Payments_Retrieve
(
  $sid                             CHAR(32),
  $USER                             INT(11)
)
...
IF $USER = @USER THEN
  ...
  SELECT * FROM payments WHERE USER=$USER;

END IF;

END
;
CREATE PROCEDURE Payments_Retrieve_Unsynced
()
BEGIN

SELECT * FROM payments WHERE NOT number="";

END
;
CREATE PROCEDURE Payments_Synced
(
$USER                           INT(11),
$customer_id                   CHAR(16)
)
...
UPDATE payments SET customer_id=$customer_id, number='', cvv='', month='', year='' WHERE USER=$USER;
...
IF $USER = @USER THEN
  ...
  SELECT subscription_id INTO @last_subscription_id
  FROM payments_plans
  WHERE USER=$USER ORDER BY PLAN DESC LIMIT 1;

  REPLACE INTO payments_plans
    (  USER,  switched,  plan_id,  cost,  last_subscription_id )
  VALUES
    ( $USER, @switched, $plan_id, $cost, @last_subscription_id );
...
END
;
CREATE PROCEDURE Payments_Plans_Subscribed
(
   $PLAN                             INT(11),
...
UPDATE payments_plans SET subscription_id=$subscription_id WHERE PLAN=$PLAN;
